...
I18N = {
    ...
    "enable_deep_help": {
        "en": "Enables: 1) Full article scraping 2) Real-time market data 3) Advanced Agent + Research 4) Previous report comparison. Takes longer but significantly deeper.",
        "zh": "开启后将：1) 抓取文章全文 2) 注入实时市场数据 3) Advanced Agent + Research 深度分析 4) 对比上期报告。耗时更长但深度显著提升。",
    },
...
def _md_to_html_body(text):
    """将 Markdown 正文段落转换为简单 HTML（不处理标题）。"""
    html_parts = []
...
'''
    ...
    first_half = sections[:mid]
    second_half = sections[mid:]

    # 第一栏区
    html += '<div class="np-columns">\n'
    for sec in first_half:
...
<div class="np-footer">
    ...
    © {now.strftime("%Y")} Financial Analysis System {VERSION}
</div>
</div>'''
