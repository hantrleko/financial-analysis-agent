...
def test_compute_score_bullish():
    sig = AssetSignal(
        name="Test", ticker="TST", sector="equity", group="Test Group",
        price=100.0, change_1d_pct=2.0, change_5d_pct=4.0,
        change_20d_pct=8.0, above_ma20=True, volume_ratio=2.0,
    )
...
def test_compute_score_bearish():
    sig = AssetSignal(
        name="Test", ticker="TST", sector="equity", group="Test Group",
        price=100.0, change_1d_pct=-3.0, change_5d_pct=-5.0,
        change_20d_pct=-10.0, above_ma20=False, volume_ratio=2.0,
    )
    score = MarketSentimentAnalyzer._compute_score(sig)
...
def test_compute_score_vix_inverted():
    """VIX 应采用反向评分。"""
    sig = AssetSignal(
        name="VIX", ticker="^VIX", sector="volatility", group="Volatility",
        price=25.0, change_1d_pct=5.0, change_5d_pct=10.0,
        change_20d_pct=15.0, above_ma20=True, volume_ratio=1.0,
    )
    score = MarketSentimentAnalyzer._compute_score(sig)
...
def test_build_reason():
    sig = AssetSignal(
        ...
        price=500.0, change_1d_pct=1.5, change_5d_pct=3.0,
        change_20d_pct=5.0, above_ma20=True, volume_ratio=2.0,
    )
