...
**🧠 规则化分析引擎（Rule-based Engine）**
- 新增 `rule_based` 分析引擎，当未配置有效 API Key 时自动启用“无 AI”规则模式
- 规则模式采用关键词命中计分（正向 / 负向）与板块映射，产出可读的情绪聚合、快照解读、行动清单
- 流式分析路径也支持规则兜底，无需等待 LLM 返回即可继续展示结果
- 处理流程新增 `provider` 可用性判断，避免因缺少 Key 直接抛错导致页面空白

### Fixed — 规则模式稳定性修复

**📈 市场快照解析修复**
- 修复 `_parse_snapshot` 正则转义导致的解析失败，确保 5 日快照行可稳定提取涨跌幅

### Changed
- 版本号升级：`v2.2` → `v2.3`
- 分析引擎配置中新增 `llm_rule_mode` 提示文案，前端在选择“无 API 模式”时自动提示无需模型即可运行
